...
const NestedCollapseWithTabs = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(baseUrlofAcl + "/test"); // Adjust the URL if needed
  ...
  const handleOkDeleteServiceName = async () => {
    ...
    try {
      const response = await axios.post(
        baseUrlofAcl + "/deleteAclApiServiceNameRequest",
        {
  ...
  const handleInputChange = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(baseUrlofAcl + "/aclApiServiceName", {
  ...
  return (
    <MainDivwithoutSidebar>
      <div style={{ margin: "0px 15px" }}>
        <h2 style={{ fontSize: "22px", fontWeight: 700 }}>Service Mapping</h2>
        <FormData
      ...
      <Table dataSource={data} columns={columnsAcl} />
    </MainDivwithoutSidebar>
  );
};
