...
const Chart = ({ chartValues }) => {
  return <ReactECharts option={chartValues} style={{ height: "450px" }} />;
};
...
const Index = () => {
  ...
  const handleApplyClick = async () => {
    ...
    console.log(result);
    const updatedData = [...tempData, result];
  ...
  const handleSaveTemplate = () => {
    ...
    // Save the current template (templatesOption) with the given name
    const newTemplate = { name: templateName, options: options };
    setTemplates([...templates, newTemplate]);
    setTemplateName("");
  };
  // const handleResetTemplate = () => {
  //   setTemplatesOption(null);
  //   handleCancelTemplateGraph();
  //   // setIsModalOpenTemplateGraph(false);
  // };
  const handleCancelTemplateGraph = () => {
    handleResetTemplate();
  };
  ...
  //   setTemplatesOption(template.options);
  // };
  const [isModalOpenTemplateGraph, setIsModalOpenTemplateGraph] =
    useState(false);
  const showModalTemplateGraph = () => {
    setIsModalOpenTemplateGraph(true);
  };
  const handleOkTemplateGraph = () => {
    setIsModalOpenTemplateGraph(false);
    setTemplatesOption(null);
  };
  // const handleCancelTemplateGraph = () => {
  ...
  //   setTemplatesOption(null);
  // };
  const [templatesOption, setTemplatesOption] = useState(null); // Change initial state to null
  ...
  useEffect(() => {
    if (templatesOption !== null) {
      updateChart(templatesOption);
    }
    console.log("--------------");
  }, [templatesOption]);

  // const updateChart = (chartData) => {
  //   return <Chart chartValues={chartData} style={{ height: "450px" }} />;
  // };
  ...
  //   const legendData = templateOptions.series.map(
  //     (series) => series.name || series.data.name
  //   );
  ...
  //     ...templateOptions,
  //     legend: {
  //       ...templateOptions.legend,
  //       data: legendData,
  //     },
  //     xAxis: {
  //       type: "category",
  //       data: templateOptions.xAxis.data,
  //     },
  //     yAxis: {
  //       type: "value",
  //     },
  //     series: templateOptions.series,
  //   };
  //   console.log(updatedOptions);

  //   // Set the new data in setTemplatesOption
  //   setTemplatesOption(updatedOptions);
  // };

  const handleLoadTemplate = (template) => {
    console.log(templatesOption);
    showModalTemplateGraph();
    console.log(template);
    const templateOptions = template.options;
    console.log(template.options);
    const legendData = templateOptions.series.map(
    ...
    // Update the name property of each series to match the legend data
    const updatedSeries = templateOptions.series.map((series, index) => series);
    ...
    <MainDivwithoutSidebarInfluxgraph>
      <div className="btncontainer">
        {isGraphVisible && (
          <>
            <Popconfirm
              title="Are you sure you want to reset the Graph?"
              onConfirm={handleResetClick}
              okText="Yes"
              cancelText="No"
            >
              <Button type="primary" danger style={{ marginTop: "7px" }}>
                Reset the Graph
              </Button>
            </Popconfirm>{" "}
            {/* &nbsp;&nbsp;
      ...
      >
        <div>
          ...
          >
            ...
            />
            <Button type="primary" onClick={() => handleSaveTemp()}>
              Save Template
            </Button>
          </Space.Compact>
          ...
          {/* <button onClick={handleResetTemplate}>reset Template</button> */}
        </div>
      </Modal>
      {/* <div>
        ...
        <ul>
          {templates.map((template, index) => (
            <li key={index} onClick={() => handleLoadTemplate(template)}>
              {template.name}
            </li>
          ))}
        </ul>
      ...
      {/* {templatesOption !== null ? updateChart(templatesOption) : null} */}
      <Modal
        title="Template"
        open={isModalOpenTemplateGraph}
        onOk={handleOkTemplateGraph}
        onCancel={handleCancelTemplateGraph}
        width={750}
        maskClosable={false}
        footer={false}
      >
        <div style={{ height: "450px", width: "650px" }}>
          {templatesOption !== null ? (
            <>
              <ReactECharts
      ...
      <Row>
        ...
        {/* <Col xs={6} sm={6} md={4} lg={4} xl={4}>
          ...
          >
            ...
            {allTempList.map((template, index) => (
              <li key={index} style={{ fontSize: "18px", fontWeight: 600 }}>
                {template}
              </li>
            ))}
          </ul>
            ...
            >
              {cards.map((card, index) =>
                clickedCard !== null ? (
                  // <Popconfirm
                  //   key={index}
                  //   title={`Previous data will be discarded, Do you want to continue?`}
